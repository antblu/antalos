...
resource "proxmox_virtual_environment_vm" "debian" {
  ...
  description     = "Debian 13.6 CLI VM with Intel Arc A310 passthrough; managed by OpenTofu"
  tags            = ["debian", "gpu", "arc-a310"]
  on_boot         = true
  started         = true
  ...
  dynamic "hostpci" {
    for_each = var.arc_a310_passthrough_enabled ? [var.arc_a310_mapping] : []
