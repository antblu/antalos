...
The [Debian VM layer](/infrastructure/virtual-machines/) has its own availability limits. Arc hosts recording, Immich Machine Learning, Jellyfin, LazyLibrarian, Storyteller, and Docling. RTX hosts llama-swap and Speaches. The left playbook configures a base Debian host without deploying Compose workloads. Each managed VM endpoint is a singleton; its loss interrupts its consumers' corresponding features. Manually installed applications require a separate inventory.
