...
flowchart TB
    ...
    Tofu[OpenTofu] --> PX[Three Proxmox hosts]
    PX --> CPL[talos-control-left]
    PX --> CPR[talos-control-right]
    PX --> CPQ[talos-control-rtx]
    PX --> WL[talos-worker-left]
    PX --> WR[talos-worker-right]
    PX --> WQ[talos-worker-rtx]
    CPL & CPR & CPQ --> Etcd[Three-member control plane and etcd quorum]
    WL & WR --> Apps[Primary application and stateful workloads]
    WQ --> Quorum[Lightweight quorum voters]
    Apps --> Local[OpenEBS local volumes]
    Apps --> External[External NFS and Garage S3 at 10.30.0.5]
```
...
| --- | --- | --- |
| OpenEBS LocalPV hostpath | PostgreSQL, Redis, Elasticsearch, VictoriaMetrics, MariaDB, and recording spools | A volume is local to one worker and is not replicated by OpenEBS. Availability comes from the database or application maintaining another copy on the other worker. |
...
OpenTofu creates the Talos VMs, applies machine configuration, bootstraps etcd, writes `kubeconfig`, and installs the initial Argo CD release. Argo CD then reconciles the root app-of-apps and every directory under `apps/`.
...
- intentionally single-instance workloads listed in [Service availability](/architecture/service-availability/).
