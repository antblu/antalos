...
Open [flows.antblu.net](https://flows.antblu.net) and sign in through Node-RED's Authentik OIDC button. Access requires an administrator-assigned policy. Use the editor to build and deploy flows; the server stores flow definitions and installed nodes in its persistent `/data` directory.
