---
title: "RustDesk networking and recovery"
description: "Native and WebSocket endpoints, relay session affinity, NFS permissions, and safe single-writer recovery."
---

Start with the [RustDesk deployment guide](/admin-guide/rustdesk/) and [user setup](/user-guide/rustdesk/). This companion runbook preserves the exact network and persistence contract for the OSS server.
...
These addresses are private LAN addresses. For remote access, provide reachable routing or public DNS/NAT for every advertised endpoint. With a single public IP, the two relays need distinct external ports and corresponding changes to their advertised endpoints. The shared Traefik endpoint already advertises distinct relay ports 21117 and 21120; the direct LoadBalancer endpoints are additional paths that require their own reachability if used. The pod network must also resolve and reach the advertised relay endpoints for hbbs health detection.
...
Treat these as design expectations until an administrator records a controlled recovery drill. Record observed interruption, backup lag, lock release, and client reconnection time instead of promising a fixed recovery interval.
